...
## Hand off to the bench sheet

As the final step, invoke `create-bench-sheet` to render the completed manifest into a standalone human-readable bench sheet for the person who will run the experiment, and show it to the user.

Do this even when the experiment did not reach `READY_TO_RUN`; that skill marks unresolved items in place rather than omitting them.

Writing the sheet forces every step into an executable sequence and so tends to expose gaps that section-by-section preflight did not. When `create-bench-sheet` reports one, record it in the manifest's preflight findings and reconsider whether the experiment's state is still justified.
